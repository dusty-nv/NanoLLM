...
class CLIPImageEmbedding():
    ...
    """
    @staticmethod
    def from_pretrained(model="openai/clip-vit-large-patch14-336", dtype=torch.float16, use_cache=True, **kwargs):
    ...
    def __init__(self, model, dtype=torch.float16, **kwargs):
        ...
        if not hasattr(self, 'model_type'):
            raise ValueError(f"tried loading vision encoder {model} - supported model types are CLIP and SigLIP")
            
        logging.info(f'loading {self.model_type} model {model}')
    ...
    def __call__(self, image, crop=False, hidden_state=None, return_tensors='pt', **kwargs):
        return self.embed_image(image, crop=False, hidden_state=None, return_tensors='pt', **kwargs)
