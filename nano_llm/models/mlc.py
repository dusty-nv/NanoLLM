    ...
    def _generate(self, stream):
        ...
        while True:
            ...
            if self.kv_cache_paged:
                ...
                embedding = self._embed(tvm.nd.array(np.array([[stream.tokens[-1]]], dtype=np.int32), self.device), self.params)
                ...
                output = self._decode(
                    tvm.nd.array(np.array([[stream.tokens[-1]]], dtype=np.int32), self.device),
