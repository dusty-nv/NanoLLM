...
class MLCModel(NanoLLM):
    ...
    def generate(self, inputs, streaming=True, plugins=None, **kwargs):
        """
        Parameters:
          ...
          plugins (list[Plugin|callable]) -- Sampler functions or plugins to run inline with generation 
                                             for things like function calling, guidance, token healing, ect.
                                             These will be passed the text generated by the LLM, and any
        ...
        for idx, plugin in enumerate(plugins):
            if not isinstance(plugin, Plugin):
                if not callable(plugin):
                    raise TypeError(f"plugins passed to NanoLLM.generate() should be either a callable function, or a Plugin instance (was {type(plugin)})")
            plugins[idx] = Callback(plugin)
        '''
        
        stream = StreamingResponse(self, inputs, plugins=plugins, **kwargs)
    ...
    def _generate(self, stream):
        ...
        """
        plugins = stream.kwargs.get('plugins', [])
        ...
        while True:
            ...
            # TODO change `token` above to a list and then add any tokens made by plugins to it
            # (and adapt the code below to handle > 1 token at a time)
            plugin_text = ''
            
            for plugin in plugins:
                plugin_next = plugin(stream.text)
                if not plugin_next:
                    continue
                plugin_text = ' ' + plugin_next
                
            if plugin_text:
                plugin_tokens = self.tokenize(plugin_text)[0].tolist()
                tokens.extend(plugin_tokens)
                stream.add_tokens(plugin_tokens)
